...
| 5   | **TC-14 stap 1** (bij codereview, niet in de testrun): het testplan verwacht dat de tabel op /beheer vóór het anonimiseren beide logs met de naam van het lid toont. `haalBezoeklogs` in `src/server/beheer/anonimiseren.ts` toont een log van vóór de grens van 14 dagen echter al als "Anoniem", ook als het anonimiseren nog niet is uitgevoerd ("Zo klopt het overzicht al vóór er op 'Anonimiseren' is geklikt"). De log van 21 dagen oud toont dus al "Anoniem" vóór stap 2 | [AANVULLEN: conclusie na handmatige controle — gedrag accepteren en testplan verduidelijken, of weergave aanpassen]    |
...
| Elke afwijking heeft een conclusie: opgelost, geaccepteerd met onderbouwing, of doorgeschoven als verbetervoorstel | **Niet voldaan**             | §5.1 en §5.2 hebben allemaal een conclusie. Voor §5.3 punt 1 (TC-08 stap 5) en punt 5 (TC-14 stap 1) is de conclusie nog [AANVULLEN]            |
...
- De afwijking bij TC-08 stap 5 (einddatum één dag eerder dan het testplan beschrijft) heeft nog
  een conclusie nodig, net als de bevinding bij TC-14 stap 1 (oude log al "Anoniem" vóór het
  anonimiseren, §5.3 punt 5).
