...
| 5   | **TC-14 stap 1**: het testplan verwacht dat de tabel op /beheer vóór het anonimiseren beide logs met de naam van het lid toont. Bij de handmatige controle toonde de log van 21 dagen oud al "Anoniem". Nagecontroleerd op een verse seed: de status meldt nog 1 log met een lidnummer, maar de tabel toont die log als "Anoniem". Oorzaak: `haalBezoeklogs` in `src/server/beheer/anonimiseren.ts` maskeert elke log van vóór de grens van 14 dagen, ook als `user_id` nog gevuld is ("Zo klopt het overzicht al vóór er op 'Anonimiseren' is geklikt") | [AANVULLEN: conclusie — gedrag accepteren en testplan verduidelijken, of weergave aanpassen]                                               |
| 6   | **TC-04 stap 2**: het testplan verwacht dat het weigeringsscherm vermeldt wanneer het lid weer terecht kan. Het scherm toont alleen de vaste tekst "Kom volgende week terug" (`app/check-in/_components/Resultaat.tsx`), zonder concrete dag of datum. Door de tester beoordeeld als te vaag: niet conform                                                                                                                                                                                                                                               | [AANVULLEN: conclusie — geaccepteerd met onderbouwing, of verbetervoorstel (bijv. de concrete dag tonen waarop de telling opnieuw begint)] |
...
| Elke afwijking heeft een conclusie: opgelost, geaccepteerd met onderbouwing, of doorgeschoven als verbetervoorstel | **Niet voldaan**             | §5.1 en §5.2 hebben allemaal een conclusie. Voor §5.3 punt 1 (TC-08 stap 5), punt 5 (TC-14 stap 1) en punt 6 (TC-04 stap 2) is de conclusie nog [AANVULLEN]                                                                       |
...
- De afwijkingen bij TC-08 stap 5, TC-14 stap 1 en TC-04 stap 2 hebben nog een conclusie nodig.
- NF-01 is niet gerealiseerd. /beheer is voor iedereen bereikbaar. Dat is bewust buiten de scope
  van de kernflow gehouden.

[AANVULLEN: eindoordeel na de conclusies bij §5.3 punt 1, 5 en 6]
